...
const FadderHomePage: React.FC = () => {
    ...
    if (loading && !useSampleData) return <Text>Loading...</Text>;
    ...
    return (
        <Container fluid p="xl" style={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
            <Stack spacing={0} style={{ flex: 1, overflow: 'hidden' }}>
                <Text size="xl" weight={500}>Senaste Nytt!</Text>
                <ScrollArea style={{ flex: 1 }}>
                    <Stack spacing="sm">
                        {tasks.map((task, index) => (
                            ...
                            >
                                <Stack spacing="xs">
                                    <Text size="md" weight={700} color="white">{task.title}</Text>
                                    <Text size="sm" color="white">{task.description}</Text>
                                    <Group spacing="xs">
                                        <Text size="xs" color="dimmed">{task.author}</Text>
                                        <Text size="xs" color="dimmed">{formatDateTime(task.created_at)}</Text>
                                    </Group>
                                </Stack>
                            </Paper>
                        ))}
