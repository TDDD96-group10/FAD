...
interface EditFadderOverviewProps {
  id: string;
}


  
const EditFadderOverview: React.FC<EditFadderOverviewProps>  = ({id:string}) =>{
   const { data: user, loading, error, setField, setValue } = useApi(() => apiClient.portal.portalProfileMetaDataList());
     ...
     function getTags() {
       ...
       return <MultiSelect
          ...
          data={tags}
          value={Array.isArray(jsonStringUser["tags"]) ? jsonStringUser["tags"]  : []}
          onChange={(selectedValues) => {
            console.log(selectedValues)
            if (user?.attributes) {
              console.log(selectedValues)
              setField("attributes", { ...user?.attributes, ["tags"]: selectedValues })
            }
          }}
          searchable
          clearable
...
export default EditFadderOverview;
